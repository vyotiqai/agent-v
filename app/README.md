...
|---|---|---|
| The shell | `src/App.tsx`, `index.ts` | Loads the fonts and the session while the splash screen shows, follows the phone's light or dark appearance (D69), keeps the window behind the app the ground colour, and shows a plain message if a screen fails to draw. Screens come with the slices that make them work (stage 7) |
| Screens (slice 1) | `src/screens/` | Welcome, when signed out; Privacy and your data, when signed in, showing your phones and Sign out (D137, D138) |
| The session (slice 1) | `src/auth/`, `src/api/client.ts` | Signing in (a nonce from the API, a new signing key, Google's own sign-in sheet, then the API), the refresh token in the secure store and the access token only in memory, one refresh at a time, and back to Welcome when the API says the phone is signed out |
| Native modules (slice 1) | `modules/google-sign-in`, `modules/device-key` | Our own small Kotlin modules: Android's sign-in sheet (Credential Manager) for a Google identity token carrying the nonce; a P-256 key made in the phone's secure chip (StrongBox where there is one), usable only after the fingerprint, face or PIN, never leaving the phone. iPhone versions come with slice 13 |
...
## Settings a build is made with

| Setting | What it is |
|---|---|
| `EXPO_PUBLIC_API_URL` | The API's address (staging or production) |
| `EXPO_PUBLIC_GOOGLE_CLIENT_ID` | Agent V's server client id in Google Cloud, which Google's sign-in sheet issues tokens for |
| `AGENT_V_ANDROID_PACKAGE` | The Android package name. It follows the domain (D124), so until that is chosen each build names it; CI's compile check uses `ci.agentv.check` |

## Running it

The native project (`android/`) is generated by `npx expo prebuild`, never kept. CI generates and
compiles it on every change. A development build on a real phone needs the settings above, and the
real package name, set with the domain (stage 7, D124).
