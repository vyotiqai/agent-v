...
| The recommended models and their prices | `server/src/ai/catalog.ts` | Anthropic's in; OpenAI's and Google's chosen from the first real run |
...
| A declined key, against the real providers from GitHub's machines (`server/live/`) | Runs in the Providers check; needs no key |
